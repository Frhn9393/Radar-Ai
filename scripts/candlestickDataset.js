function parseDatasetCsv(contents, header) {
    ...
    return lines.map(line => line.split(',')).filter(values => values.length === header.length).map(values => {
        if (header.some((name, index) => name !== 'ticker' && name !== 'date' && !values[index].trim())) return null;
        const row = Object.fromEntries(header.map((name, index) => [name, values[index]]));
        for (const name of header) {
            if (name !== 'ticker' && name !== 'date') row[name] = Number(row[name]);
        }
        return row;
...
function isValidDatasetRow(row, header) {
    ...
    if (header.some(name => name !== 'ticker' && name !== 'date' && (row[name] === null || row[name] === undefined || String(row[name]).trim() === ''))) return false;
    const requiredPrices = ['open', 'high', 'low', 'close', 'volume', 'labelWin'];
    ...
    if (Number(row.low) > Math.min(Number(row.open), Number(row.close)) || ![0, 1].includes(Number(row.labelWin))) return false;
    return header.every(name => name === 'ticker' || name === 'date' || Number.isFinite(Number(row[name])));
}
...
function serializeDatasetCsv(rows, header) {
    return `${[header.join(','), ...(Array.isArray(rows) ? rows : []).map(row => header.map(name => {
        ...
        return name === 'ticker' || name === 'date' ? String(value) : String(Number(value));
    }).join(','))].join('\n')}\n`;
}
