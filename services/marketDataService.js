...
// Share identical quote requests during the short bursts generated by dashboards
// and screeners; Yahoo quote freshness is still preserved with a 20 second TTL.
const quoteCache = new Map();
const quoteInFlight = new Map();
const QUOTE_CACHE_TTL = 20 * 1000;
const MAX_QUOTE_CACHE_ENTRIES = 500;

async function getYahooQuote(symbol) {
    const cached = quoteCache.get(symbol);
    if (cached && Date.now() - cached.cachedAt < QUOTE_CACHE_TTL) return cached.value;
    if (quoteInFlight.has(symbol)) return quoteInFlight.get(symbol);

    const request = yahooFinance.quote(symbol).then(value => {
