...
let indicesCacheRef = null;
function setIndicesCacheRef(cache) {
    indicesCacheRef = cache;
}

async function getUsdIdrRate() {
    if (indicesCacheRef && indicesCacheRef.usdidr && indicesCacheRef.usdidr.price) {
        return indicesCacheRef.usdidr.price;
    }
    try {
...
async function get_financial_report(ticker) {
    ...
    // - Book Value (BVPS) dari Yahoo Finance disajikan dalam USD,
    //   sehingga HARUS dikonversi ke IDR menggunakan kurs real-time USD/IDR.
    // - Sedangkan EPS dari Yahoo Finance untuk saham .JK umumnya SUDAH disajikan dalam IDR.
